function mentorRespRequest(state,id,code,tel){
...
function menteeRespRequest(state,id,code,tel){
	console.log("respRequest : "+state + ","+ code);
	$.ajax({
		url : "responsRequest",
		type : "post", 
			...
			if(data.listcount > 0 ){
				...
				$(data.reqlist).each(function(index,item){
				  output +='<tr><td><div class="classalign">'+item.sports_name+'</div></td>'
					     ...
					     +'<td colspan="3"><div class="classalign">'+item.user_name+"/"+item.user_mobile+'</div></td>';
			      		 if(item.match_state == 1){
			      			output+='<td><div onclick="javascript:mentorRespRequest(2,'+"'"+item.user_id+"',"+"'"+item.match_code+"',"+"'"+item.user_mobile+"'"+');" class="submit4">요청수락</div></td>'
							       +'<td><div onclick="javascript:mentorRespRequest(3,'+"'"+item.user_id+"',"+"'"+item.match_code+"',"+"'"+item.user_mobile+"'"+');" class="submit4">요청거절</div></td></tr>'
					     }else if(item.match_state == 2){
				      			output+='<td><div  class="submit6">수락완료</div></td>'
							       +'<td></td></tr>'
					     }else if(item.match_state == 3){
				      			output+='<td></td>'
							       +'<td><div  class="submit6">거절완료</div></td></tr>'
					     }
			...
			if(data.listcount > 0 ){
				...
				$(data.reqlist).each(function(index,item){
					  output +='<tr><td><div class="classalign">'+item.sports_name+'</div></td>'
						     ...
						     +'<td colspan="3"><div class="classalign">'+item.user_name+"/"+item.user_mobile+'</div></td>';
				      		 if(item.match_state == 1){
				      			output+='<td><div onclick="javascript:menteeRespRequest(2,'+"'"+item.user_id+"',"+"'"+item.match_code+"',"+"'"+item.user_mobile+"'"+');" class="submit4">요청수락</div></td>'
								       +'<td><div onclick="javascript:menteeRespRequest(3,'+"'"+item.user_id+"',"+"'"+item.match_code+"',"+"'"+item.user_mobile+"'"+');" class="submit4">요청거절</div></td></tr>'
						     }else if(item.match_state == 2){
					      			output+='<td><div class="submit6">수락완료</div></td>'
								       +'<td></td></tr>'
						     }else if(item.match_state == 3){
